...
def explore():
	...
	return render_template('blog/index.html', title='Explore', posts=posts.items,next_url=next_url,prev_url=prev_url)
...
def index():
	...
	return render_template("blog/index.html", title='Home Page', form=form,posts=posts.items,next_url=next_url,
								prev_url=prev_url)
...
def edit_profile():
    ...
    return render_template('blog/edit_profile.html', title='Edit Profile',
                           form=form)
...
def user(username):
	...
	posts = user.posts.order_by(Post.timestamp.desc()).paginate(
		page, app.config['POSTS_PER_PAGE'], False)
	next_url = url_for('blog.user', username=user.username, page=posts.next_num) if posts.has_next else None
	prev_url = url_for('blog.user', username=user.username, page=posts.prev_num) if posts.has_prev else None
